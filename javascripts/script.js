...
$(document).ready(function(){
  ...
  ga('send', 'event', 'Button', 'Click', 'generate');
     ...
     var inputs = _.map(ids, function(id, i) {
       var $select = $("#"+id);
       var num = $select.val();
       var options = _.join(_.slice(array, _.shuffle(data[i]), 0, num));
       formdata[columns[i]] = options;
       $("#"+columns[i]).html(options);
       results.push("(" + options + ")");
       return num;
     });
     formdata["input"] = _.join(inputs);
     ga('send', 'event', PTTID, _.join(inputs), _.join(results));
